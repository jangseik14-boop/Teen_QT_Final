...
## 3. GitHub 저장소 확인 및 브랜치 체크
- 현재 GitHub의 **`Teen_QT`** 저장소에 코드가 올라가 있는지 확인하세요.
- **브랜치 이름 확인**: `Teen_QT` 저장소 페이지에 들어가면 왼쪽 상단 드롭다운 버튼에 `main` 또는 `master`라고 적혀 있습니다. (보통 `main`입니다.)
...
## 6. 비용 및 관리
- `apphosting.yaml`에 `minInstances: 0`으로 설정되어 있어 사용자가 없을 때는 비용이 거의 발생하지 않습니다.
